              ...
              <app-card class="hover:shadow-lg transition-all duration-300">
               ...
               <app-card class="hover:shadow-lg transition-all duration-300">
                <div class="p-4 sm:p-6 text-center">
                  ...
                  <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2 transition-colors">Payslip</h3>
                  <p class="text-sm text-gray-500 dark:text-gray-400 mb-4 transition-colors">View your payslip and download it</p>
                  <app-reusable-button
                    text="View Payslip"
                    variant="secondary"
                    size="sm"
...
export class DashboardComponent implements OnInit {
  ...
  viewReports(): void {
    ...
    this.router.navigate(['/employee/view-reports']).then(
      (success) => {
        if (success) {
          console.log('Navigation to employee reports successful');
          this.toastService.success('Navigated to Employee Reports');
        } else {
          console.error('Navigation to employee reports failed');
        }
      }
    );
  }
  ...
  viewPayslip(): void {
    this.router.navigate(['employee/view-payslip']).then(
    (success)=>{
      if(success){
        console.log('Navigation to employee payslip successful');
        this.toastService.success('Navigated to Employee Payslip');
      }else{
        console.error('Navigation to employee payslip failed');
        }
    },
    (error)=>{
      console.error('Error during navigation to employee payslip:', error);
      this.toastService.error('Failed to navigate to Employee Payslip');
      }
    )
  }
